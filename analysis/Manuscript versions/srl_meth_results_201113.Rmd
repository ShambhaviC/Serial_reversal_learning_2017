...
In this serial reversal learning task the bats had to choose between a flower that gave 40 $/mu$L of nectar (S+) and one that gave no reward at all. The spatial locations of the two flowers served as the cues indicating which flower was more profitable. After a bat had made 50 visits in total to the two flowers a reversal occurred: the previously rewarding flower became the non-rewarding flower and vice versa. The batch of 50 visits that occurred between two consecutive reversals (when the location of the S+ and S- stayed constant) was termed a 'reversal block', including the first 50 visits of a night when the . This occurred at regular intervals of 50 visits until the bat either stopped making visits or reached a maximum of 300 visits in a night. After the bat had made 300 rewarded visits it could no longer receive a reward that experimental night. There were 5 reversals per night. This stage of the experiment was repeated for 3 nights in a row. Each experimental night was called a 'Day'. The same flower was the first to be rewarding at the start of every Day. Thus, because there were 6 reversals every night, the last flower to be rewarding on a night was non-rewarding at the start of the next night. 

## Analysis 

This bit to be written a little later. 

# Results - need to transfer all this to a presentation for Alex 
...
```
## The bats show a high preference for S+ and sometimes nearly 'error'-free performance 
Overall, the vast majority of visits made by the bats were  to the S+, i.e., the option that is rewarding at any given time, as shown in Figure 1. Immediately after a reversal occurs, the bats make more visits to the option that was previously rewarding, and is now the S-, and then rapidly change this preference to the newly rewarding S+. Some of the bats did not complete the required 300 visits every night, and thus the later reversal blocks on all 3 Days have data from fewer than 14 bats. No reversal block had data from fewer than 10 bats, although this was not necessarily the same 10 bats on all 3 Days. 

CHANGE FIGURE ONE TO SLIDES 2 AND 3 OF THE PRESENTATION 
```{r}
#----------------------------------------
...
rev_learning_all <- raw_data %>%
  ...
  mutate(bins = as.numeric(cut(block_vis, breaks, include.lowest = TRUE)), 
         bin_count = as.numeric(cut(count_vis, breaks, include.lowest = TRUE)))
...
# creating a data frame with the individual alpha bats and the three main Days of the experiment
rev_learning_ind <- rev_learning_all %>%
  filter(!Bat %in% bats_beta) %>% 
  group_by(Day, Bat, block) %>% 
  # calculating the average preference for the S+ within each block
  group_by(Day, Bat, block) %>% 
  mutate(block_avg = mean(reward_status)) %>% 
  # calculating the average preference for the S+ within each bin 
  group_by(Day, Bat, block, bins) %>% 
  mutate(bin_avg = mean(reward_status)) 
...
```
THE FIGURE BELOW - NOT SO NECESSARY? 
```{r, fig.cap = "Figure 1: a) Average preferences of the bats for the rewarding option; b) Average preferences of the bats for the less rewarding option - the 'errors'", fig.width = 12, fig.height = 3}

#--------------
# Summary plots
#--------------
# plot 1a: averaging the bats' preferences over Day, block and bin
...
p1 <- rev_learning_avg %>%
  ...
  geom_hline(yintercept = 0.95, linetype = "dotted", colour = "red") + 
...
# plot 1b: plotting the individual bats' choices for the less profitable option, the inverse of plot 1
...
##Effect of day, block and bin on the bats' preference for the S+

The fixed effects day and bins significantly influenced the preference of the bats, as did the interaction of reversal block with bin, but the effect block and the interaction of day and block did not (see Table 1). The bats' choice for the rewarding option increased within each reversal block, from a high number of choices for the S- in the bin immediately after the reversal to near 100% preference for the S+ as the reversal block progressed. However, their overall preference for the S+ within each block decreased as they experienced successive blocks. 
...
kable(summary_prof, align = "l", 
      padding = 0, 
      caption = "Table 1: Results of the Bayesian Generalized Linear Mixed Model testing for the effect of Day, block, bin (fixed effects) and Individual (random effect) on choice for the S+ (dependent variable)") %>%
  kable_styling(position = "center", full_width = F)
```
Immediately after a reversal the bats made visits to the flower that was previously the S+ to learn that this was no longer the rewarding flower. Following this, for the rest of the block, relatively few visits were made to the S- flower, as the preference of the animals reached an asymptote. The asymptotic phase was defined as beginning when the average preference within each bin was never lower than the average preference for that block for more than two consecutive bins. Each bat reached an asymptotic phase within each block for all three days of the experiment (see Figure 2).
CHANGE THE ABOVE TO THE PROPER INTERPRETATION 
MOVE THE FIGURE BELOW TO THE SUPPLEMENT
```{r, fig.cap="Figure 2: All individual choices of the bats for the rewarding and non-rewarding options", fig.width = 12, fig.height = 7}

# plot 2: plotting the individual bats' choices for the profitable option for every visit
...
rev_main <- rev_learning_ind %>%
  ...
  select(Day, Bat, count_vis, block, bins, bin_count, MsgValue1)
...
# plot 3: plotting the bin averages and asymptotic phases for each bat 

rev_block_avg <- rev_learning_ind %>% 
  ungroup() %>%  
  select(Day, Bat, block, block_avg) %>% 
  distinct()

rev_learning_ind %>% 
  filter(Day %in% main_days) %>% 
  ggplot(aes(bin_count, bin_avg)) + 
  geom_line() + 
  geom_point() + 
  facet_grid(Bat ~ Day) + 
  geom_vline(aes(xintercept = bin_count), rev_main, colour = "red") + 
  #geom_segment(aes(x = y = xend = yend = ), data = rev_block_avg, colour = "black", linetype = #"dashed")
  ylab("Bin average") + 
  xlab ("Bins") + 
  theme_classic()
...
```
## The steady improvement through increased experience of reversals, leaving out day 

## The first block/bin of a day - double check this 

## The number of visits until the bats switched to the other option and how the experience of reversals changed that 
FIGURE FROM SLIDE 4 CAN GO IN HERE 

## 'Win-stay, lose-shift': the bats that showed near-perfect rule-learning 
